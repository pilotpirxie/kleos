...
} & Record<string, KleosHeaderValue | Record<string, KleosHeaderValue> | undefined>;
/**
...
export interface ParamsSerializerOptions {
    ...
    serialize?: (params: Record<string, unknown>, options?: ParamsSerializerOptions) => string;
...
export interface FormSerializerOptions {
    visitor?: (value: unknown, key: string, path: string | null, helpers: FormSerializerHelpers) => boolean;
    dots?: boolean;
    metaTokens?: boolean;
    indexes?: boolean | null;
}
...
export interface FormSerializerHelpers {
    defaultVisitor: (value: unknown, key: string, path: string | null, helpers: FormSerializerHelpers) => boolean;
 ...
 */
export type KleosRequestTransformer = (data: unknown, headers: Record<string, string>) => unknown;
/**
 * Mutates parsed response data after the wire read so apps can normalize dates or unwrap API envelopes.
 */
export type KleosResponseTransformer = (data: unknown) => unknown;
/**
...
export interface KleosRequestConfig<D = unknown> {
    ...
    paramsSerializer?: ParamsSerializerOptions | ((params: Record<string, unknown>) => string);
    data?: D;
    timeout?: number;
 ...
 */
export interface InternalKleosRequestConfig<D = unknown> extends KleosRequestConfig<D> {
...
export interface KleosInstance extends Kleos {
    <T = unknown, R = KleosResponse<T>, D = unknown>(config: KleosRequestConfig<D>): Promise<R>;
    <T = unknown, R = KleosResponse<T>, D = unknown>(url: string, config?: KleosRequestConfig<D>): Promise<R>;
    ...
    request<T = unknown, R = KleosResponse<T>, D = unknown>(config: KleosRequestConfig<D>): Promise<R>;
    get<T = unknown, R = KleosResponse<T>, D = unknown>(url: string, config?: KleosRequestConfig<D>): Promise<R>;
    delete<T = unknown, R = KleosResponse<T>, D = unknown>(url: string, config?: KleosRequestConfig<D>): Promise<R>;
    head<T = unknown, R = KleosResponse<T>, D = unknown>(url: string, config?: KleosRequestConfig<D>): Promise<R>;
    options<T = unknown, R = KleosResponse<T>, D = unknown>(url: string, config?: KleosRequestConfig<D>): Promise<R>;
    post<T = unknown, R = KleosResponse<T>, D = unknown>(url: string, data?: D, config?: KleosRequestConfig<D>): Promise<R>;
    put<T = unknown, R = KleosResponse<T>, D = unknown>(url: string, data?: D, config?: KleosRequestConfig<D>): Promise<R>;
    patch<T = unknown, R = KleosResponse<T>, D = unknown>(url: string, data?: D, config?: KleosRequestConfig<D>): Promise<R>;
    postForm<T = unknown, R = KleosResponse<T>, D = unknown>(url: string, data?: D, config?: KleosRequestConfig<D>): Promise<R>;
    putForm<T = unknown, R = KleosResponse<T>, D = unknown>(url: string, data?: D, config?: KleosRequestConfig<D>): Promise<R>;
    patchForm<T = unknown, R = KleosResponse<T>, D = unknown>(url: string, data?: D, config?: KleosRequestConfig<D>): Promise<R>;
}
/**
...
export interface KleosInterceptorManager<V> {
    use(onFulfilled?: (value: V) => V | Promise<V>, onRejected?: (error: unknown) => unknown, options?: KleosInterceptorOptions): number;
 ...
 * Shallow merge would drop method-specific headers or transitional flags when users extend only one branch.
 */
function mergeDeep(a: KleosRequestConfig, b: Partial<KleosRequestConfig> | undefined): KleosRequestConfig {
    ...
    if (typeof b !== 'object' || Array.isArray(b))
        return b as KleosRequestConfig;
    ...
    for (const key of Object.keys(b)) {
        ...
        const av = out[key];
        // Recurse into nested plain objects (e.g. headers buckets) so partial updates do not erase sibling keys
        if (bv &&
            typeof bv === 'object' &&
            !Array.isArray(bv) &&
            av &&
            typeof av === 'object' &&
            !Array.isArray(av)) {
            out[key] = mergeDeep(av as KleosRequestConfig, bv as Partial<KleosRequestConfig>);
        }
...
/**
 * Reads a document cookie by name for CSRF token propagation in browser sessions.
 * Without this, state-changing requests may be rejected by frameworks that expect a matching header token.
 */
function readCookie(name: string): string | null {
    // Non-browser runtimes have no document: skip silently instead of throwing at import time
 ...
 */
function flattenHeaders(headers: RawKleosRequestHeaders | undefined, method: string): Record<string, string> {
    const out: Record<string, string> = {};
    // No headers object means “no extra headers” rather than an error state
    if (!headers)
        return out;
    const m = method.toLowerCase();
    const headerRecord = headers as unknown as Record<string, Record<string, KleosHeaderValue> | undefined>;
 ...
 */
function serializeParamsObject(params: Record<string, unknown>, options?: ParamsSerializerOptions): string {
 ...
 */
function serializeParams(params: Record<string, unknown> | URLSearchParams | undefined, paramsSerializer?: KleosRequestConfig['paramsSerializer']): string {
    ...
    return serializeParamsObject(params as Record<string, unknown>, paramsSerializer);
}
/**
 * Joins a base URL and relative path without duplicate slashes so deployed apps behind gateways get valid origins.
 */
...
/**
 * Detects absolute URLs (http(s) or scheme-relative) so baseURL merging does not double-prefix real hosts.
 */
function isAbsoluteURL(url: string): boolean {
    return /^([a-z][a-z\d+\-.]*:)?\/\//i.test(url);
}
/**
 * Chooses whether to prefix baseURL based on absolute URL rules and the allowAbsoluteUrls escape hatch.
 * Mis-merging breaks multi-tenant apps that mix CDN absolute paths with API relative paths.
 */
function buildFullPath(baseURL: string | undefined, requestedURL: string, allowAbsoluteUrls: boolean | undefined): string {
    if (baseURL && (!isAbsoluteURL(requestedURL) || allowAbsoluteUrls === false)) {
...
export function buildURL(config: InternalKleosRequestConfig): string {
    const { url = '', baseURL, allowAbsoluteUrls, params, paramsSerializer } = config;
    const path = buildFullPath(baseURL, url, allowAbsoluteUrls);
    const qs = serializeParams(params as Record<string, unknown> | URLSearchParams | undefined, paramsSerializer);
    if (!qs)
        return path;
 ...
 */
function defaultFormVisitor(value: unknown, key: string, path: string | null, helpers: FormSerializerHelpers, options: FormSerializerOptions, form: FormData): boolean {
    ...
    if (isVisitable(value)) {
        for (const [k, v] of Object.entries(value as Record<string, unknown>)) {
            ...
            defaultFormVisitor(v, k, path === null ? k : buildKey(k), helpers, options, form);
        }
        return false;
    }
    // Leaf scalar (or already-normalized) value becomes a single named part in the multipart body
    const field = path === null ? key : useDots && path ? `${path}.${key}` : `${path}[${key}]`;
 ...
 */
function objectToFormData(data: Record<string, unknown>, options?: FormSerializerOptions, FormDataRef?: new () => FormData): FormData {
    const FormDataCtor = FormDataRef ??
    ...
    const opts: FormSerializerOptions = { metaTokens: true, indexes: false, ...options };
...
/**
 * Encodes an object as x-www-form-urlencoded using the same key rules as query serialization without array indexes.
 */
function objectToUrlEncoded(data: Record<string, unknown>): string {
    return serializeParamsObject(data, { indexes: false });
}
/**
...
/**
 * Turns config.data into a Fetch body and adjusts headers for JSON, form, multipart, and binary payloads.
 * Incorrect shaping yields servers rejecting bodies, broken multipart boundaries, or double-encoded JSON.
 */
function transformRequestBody(config: InternalKleosRequestConfig): BodyInit | null | undefined {
    const { data, headers, method } = config;
    const m = normalizeMethod(method);
    // GET/HEAD should not carry serialized bodies; undefined keeps fetch from sending misleading entity bodies
    if (!isBodyMethod(m)) {
        return undefined;
    }
    ...
    if (typeof data === 'object') {
        if (ct.includes('multipart/form-data')) {
            const fd = objectToFormData(data as Record<string, unknown>, config.formSerializer, config.env?.FormData as (new () => FormData) | undefined);
            delete headers['content-type'];
            return fd;
        }
        if (ct.includes('application/x-www-form-urlencoded')) {
            setContentType(headers, 'application/x-www-form-urlencoded;charset=utf-8');
            return objectToUrlEncoded(data as Record<string, unknown>);
        }
 ...
 */
function applyTransformRequest(config: InternalKleosRequestConfig): InternalKleosRequestConfig {
    ...
    for (const fn of list) {
        ...
        data = fn(data, config.headers) as typeof data;
    }
    return { ...config, data };
}
/**
 * Reads the Response body according to responseType and transitional JSON rules the app selected.
 * Wrong parsing hides server errors as null JSON or breaks binary downloads that expect ArrayBuffers.
 */
async function parseResponseData(response: Response, config: InternalKleosRequestConfig): Promise<unknown> {
 ...
 */
function composeAbortSignals(userSignal: AbortSignal | GenericAbortSignal | undefined, timeoutMs: number, cancelToken?: CancelToken): {
    ...
    if (cancelToken?.reason) {
        ...
        dead.abort();
        return { signal: dead.signal, cleanup: () => { } };
    }
    const cleanups: (() => void)[] = [];
 ...
 */
function maybeReportUploadProgress(config: InternalKleosRequestConfig, body: BodyInit | null | undefined): void {
    const cb = config.onUploadProgress;
    if (!cb || body === undefined || body === null)
        return;
 ...
 */
async function readBodyWithDownloadProgress(response: Response, config: InternalKleosRequestConfig): Promise<unknown> {
 ...
 */
export class KleosError<T = unknown, D = unknown> extends Error implements KleosError<T, D> {
    ...
    status?: number;
    cause?: Error;
    constructor(message: string, code: string | undefined, config: InternalKleosRequestConfig<D>, request?: unknown, response?: KleosResponse<T, D>) {
        super(message);
 ...
 */
export function isKleosError<T = unknown, D = unknown>(payload: unknown): payload is KleosError<T, D> {
...
export class InterceptorManager<V> implements KleosInterceptorManager<V> {
    ...
    use(onFulfilled?: (value: V) => V | Promise<V>, onRejected?: (error: unknown) => unknown, options?: KleosInterceptorOptions): number {
        this._handlers.push({
            ...
            runWhen: options?.runWhen as ((config: InternalKleosRequestConfig) => boolean) | undefined,
        });
     ...
     */
    forEach(fn: (h: InterceptorHandler<V>) => void, config?: InternalKleosRequestConfig): void {
        for (const h of this._handlers) {
            if (!h)
                continue;
            if (config && h.runWhen && !h.runWhen(config))
                continue;
            fn(h);
        }
    }
 ...
 */
function mergeConfig(defaults: KleosRequestConfig, config: KleosRequestConfig): InternalKleosRequestConfig {
    ...
    if (merged.auth === null) {
        // Explicit null removes inherited Basic auth from defaults for public endpoints on the same client instance
        delete headers['authorization'];
    }
    return { ...merged, method, headers } as InternalKleosRequestConfig;
}
/**
 * Builds the Authorization header value for Basic auth in browsers without assuming Latin-1-only credentials.
 */
 ...
 */
async function dispatchRequest(config: InternalKleosRequestConfig): Promise<KleosResponse> {
    applyXsrfHeaders(config);
    let working = applyTransformRequest(config);
    const body = transformRequestBody(working);
    // Keep config.data aligned with the actual bytes Fetch will send after serialization decisions
    working = { ...working, data: body as unknown };
    const url = buildURL(working);
    const method = normalizeMethod(working.method);
    const timeout = working.timeout ?? 0;
    const { signal, cleanup } = composeAbortSignals(working.signal as AbortSignal | undefined, timeout, working.cancelToken);
    ...
    const redirect = working.maxRedirects === 0 ? ('manual' as RequestRedirect) : ('follow' as RequestRedirect);
    ...
    let data: unknown;
    if (working.onDownloadProgress && response.body && working.responseType !== 'stream') {
    ...
    if (transforms) {
        const list = Array.isArray(transforms) ? transforms : [transforms];
        // Response transformers run in reverse order to mirror request transformer stacking semantics
    ...
    const validate = working.validateStatus ?? ((s: number) => s >= 200 && s < 300);
    if (validate === null || validate(response.status)) {
        return kleosResponse;
    }
    throw new KleosError(`Request failed with status code ${response.status}`, 'ERR_BAD_RESPONSE', working, { kind: 'fetch', url }, kleosResponse);
}
/**
 * Runs the request/response interceptor chains around dispatchRequest, preserving promise ordering and error propagation.
 */
async function runRequest(config: InternalKleosRequestConfig, interceptors: {
    request: InterceptorManager<InternalKleosRequestConfig>;
    response: InterceptorManager<KleosResponse>;
...
export class Kleos {
     ...
     */
    postForm<T = unknown, R = KleosResponse<T>, D = unknown>(url: string, data?: D, config?: KleosRequestConfig<D>): Promise<R> {
        return this.request<T, R, D>(withMultipartFormDataHeaders('POST', url, data, config));
    }
    /**
     * PUT with multipart intent for replacing resources that include binary attachments in one round trip.
     */
    putForm<T = unknown, R = KleosResponse<T>, D = unknown>(url: string, data?: D, config?: KleosRequestConfig<D>): Promise<R> {
        return this.request<T, R, D>(withMultipartFormDataHeaders('PUT', url, data, config));
    }
    /**
     * PATCH with multipart intent for partial updates that still carry files without switching client APIs.
     */
    patchForm<T = unknown, R = KleosResponse<T>, D = unknown>(url: string, data?: D, config?: KleosRequestConfig<D>): Promise<R> {
        return this.request<T, R, D>(withMultipartFormDataHeaders('PATCH', url, data, config));
    }
}
/**
...
function createCallable(instance: Kleos): KleosInstance {
    const fn = function <T = unknown, R = KleosResponse<T>, D = unknown>(urlOrConfig: string | KleosRequestConfig<D>, config?: KleosRequestConfig<D>): Promise<R> {
...
kleos.spread =
    <T, R>(callback: (...args: T[]) => R) => (arr: T[]) => callback(...arr);
