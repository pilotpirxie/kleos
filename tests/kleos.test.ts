/**
 * Kleos compatibility harness: mirrors tests/axios.test.ts against the kleos client.
 * Uses node:test and node:assert only. Resolves kleos via package name (see devDependencies file:.).
 *
 * Intentional differences from axios.test.ts (fetch / kleos semantics):
 * - responseType "stream" yields a Web ReadableStream (not Node stream.Readable).
 */
import assert from "node:assert/strict";
import { describe, test } from "node:test";
import axios, {
  KleosError as AxiosError,
  type KleosResponse as AxiosResponse,
  isKleosError as isAxiosError,
  isCancel,
} from "kleos";
import {
  createTestFileList,
  withFileListGlobal,
} from "./helpers/file-list-test-double.js";
import {
  startTestServer,
  stopTestServer,
  type EchoedRequest,
...
describe("Response Handling & Generics", () => {
  ...
  test("responseType text returns string body", async () => {
    ...
    try {
      ...
      const textBody = res.data as string;
      assert.equal(typeof textBody, "string");
      assert.ok(textBody.includes("hello"));
    } finally {
      await stopTestServer(server);
    }
  });
  ...
  test("responseType arraybuffer returns binary buffer (ArrayBuffer or Buffer in Node)", async () => {
    ...
    try {
      ...
      // Kleos (fetch) returns ArrayBuffer; axios Node adapter may return Buffer.
      const bytes =
  ...
  test("responseType stream yields readable data (Web ReadableStream)", async () => {
    const { baseURL, server } = await startTestServer();
    try {
      const res = await axios.get(`${baseURL}/json`, {
        responseType: "stream",
      ...
      assert.ok(body && typeof body.getReader === "function");
  ...
  test("responseType blob: Blob in modern Node or Buffer fallback from adapter", async () => {
    ...
    try {
      ...
      // Kleos (fetch) uses Blob; axios Node adapter may vary.
        ...
        transformResponse: [
          (data, headers, status) => {
            ...
            sawHeaders =
              ...
              headers !== null &&
              Boolean((headers as Record<string, string>)["content-type"]);
...
describe("Payload & Form Data", () => {
  ...
  test("postForm(FileList) sends each file under files[] (Axios multipart docs)", async () => {
    if (typeof File === "undefined") {
      return;
    }
    const { baseURL, server } = await startTestServer();
    try {
      await withFileListGlobal(async () => {
        const f1 = new File(["alpha"], "one.txt", { type: "text/plain" });
        const f2 = new File(["beta"], "two.txt", { type: "text/plain" });
        const list = createTestFileList(f1, f2);
        const res = await axios.postForm<EchoedRequest>(
          `${baseURL}/echo`,
          list,
        );
        assert.ok(
        ...
        assert.ok(res.data.body.includes('name="files[]"'));
        assert.ok(
          res.data.body.includes("alpha") || res.data.body.includes("beta"),
        );
      });
    } finally {
      await stopTestServer(server);
  ...
  // Kleos flattens named controls to a JSON object (browser-oriented parity with the axios docs example).
  test("HTMLFormElement with Content-Type application/json sends JSON including field values", async () => {
    ...
    try {
      ...
      const parsed = JSON.parse(res.data.body) as Record<string, unknown>;
      assert.equal(parsed.firstName, "Fred");
      assert.equal(parsed.lastName, "Flintstone");
    } finally {
